...
const MedicineOrderDetails = ({
  ...
  const [allMEdicineOrder, setAllMEdicineOrder] = useState([]);
  ...
  const API_URL_PATIENTS = "http://localhost:3001/getAdmissionDetails";
  const API_URL_MEDICINEO = "http://localhost:3001/getMedicineOrder";
  ...
  const fetchDataMedicineOrder = async () => {
    const { data } = await axios.get(API_URL_MEDICINEO);
    setAllMEdicineOrder(data);
  };

  useEffect(() => {
    fetchData();
    fetchDoctorsData();
    fetchDataMedicineOrder();
    fetchDataOPatients();
  }, []);
  ...
  const handleAddATreatment = () => {
    if (addOnSubmit) {
      ...
      if (allMEdicineOrder.length === 0) {
        ...
        const lastElementId =
          allMEdicineOrder[allMEdicineOrder.length - 1].orderID;
        const numericPart = parseInt(lastElementId.split("_")[1]);
        const nextElementId = `O_${(numericPart + 1)
          .toString()
          .padStart(3, "0")}`;
        setInputs({
          ...inputs,
          orderID: nextElementId,
        });
      }
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post("http://localhost:3001/addMedicineOrder", inputs)
      .then((res) => {
        if (res.data === "success") {
  ...
  return (
    <div className="app-container">
      <h2 className="page-title">MEDICINE ORDER DETAILS</h2>
          ...
          <div className="container-display-infos">
            <div className="container-wrapper">
              <div className="input-field">
                <label form="schedulingId">Order ID:</label>
                <Input
                  inputDisabled="true"
              ...
              <div className="input-field doctor-types">
                <label htmlFor="patientId"> admission ID :</label>
                <div>
                  <select
                    name="admissionID"
                    id="admissionID"
                  ...
                  >
                    <option required value="">
                      Select a admission ID
                    </option>
                    {allOutPatients.map((patient, index) => (
                      <option key={index} value={patient.admissionID}>
                        {patient.admissionID}
                      </option>
                    ))}
                  </select>

...
const InPMIssueModel = require("../models/InPMedicalIssue");
const router = express.Router();

//  Create an PurchaseOrder
router.post("/addMedicalIssue", (req, res) => {
  InPMIssueModel.create(req.body)
    .then((treatment) => res.json("success"))
    .catch((err) => res.json(err));
});

// Get all the PurchaseOrder
router.get("/getMedicalIssue", (req, res) => {
  InPMIssueModel.find({ isDisplayed: true })
    .then((user) => {
      if (!user) {
        res.json("notFound");
      }

...
const MedicineCatModel = require("../models/MedicineCat");
const router = express.Router();

//  Create an out patient treatment
router.post("/addMedicineCat", (req, res) => {
  MedicineCatModel.create(req.body)
    .then((treatment) => res.json("success"))
    .catch((err) => res.json(err));
});

// Get all the Outpatients treatment
router.get("/getMedicineCat", (req, res) => {
  MedicineCatModel.find({ isDisplayed: true })
    .then((user) => {
      if (!user) {
        res.json("notFound");
      }
